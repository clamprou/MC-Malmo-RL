...
class Environment(Process):
    def __init__(self, env_idx, child_conn, agents, state_size, action_size, visualize=False):
        super(Environment, self).__init__()
        self.env = FightingZombiesDisc(agents)
...
class PPOAgent:
    ...
    def run_multiprocesses(self, num_worker = 4):
        ...
        for idx in range(num_worker):
            ...
            work = Environment(idx, child_conn, num_worker, self.state_size[0], self.action_size, True)
            work.start()
            works.append(work)
